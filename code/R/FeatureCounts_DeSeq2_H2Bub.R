...
fCount1 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Mitch_Ellison_Data_All_UPitt_2015_to_2020/ChIP_seq_Analysis_Docker_started_on_11012020/sc_bam/H2Bub_subset_bams/ME120_S6_Spt6_AID_DMSO_H2BK123ub_1_subsample_sorted.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
fCount2 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Mitch_Ellison_Data_All_UPitt_2015_to_2020/ChIP_seq_Analysis_Docker_started_on_11012020/sc_bam/H2Bub_subset_bams/ME063_S15_Spt6_AID_DMSO_H2BK123ub_2_subsample_sorted.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
# fCount3 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Mitch_Ellison_Data_All_UPitt_2015_to_2020/ChIP_seq_Analysis_Docker_started_on_11012020/sc_bam/H2Bub_subset_bams/ME051_S3_Spt6_AID_DMSO_H2BK123ub_3_subsample_sorted.bam",
#                          annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
fCount4 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Mitch_Ellison_Data_All_UPitt_2015_to_2020/ChIP_seq_Analysis_Docker_started_on_11012020/sc_bam/H2Bub_subset_bams/ME121_S7_Spt6_AID_IAA_H2BK123ub_1_subsample_sorted.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
fCount5 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Mitch_Ellison_Data_All_UPitt_2015_to_2020/ChIP_seq_Analysis_Docker_started_on_11012020/sc_bam/H2Bub_subset_bams/ME064_S16_Spt6_AID_IAA_H2BK123ub_2_subsample_sorted.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
# fCount6 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Mitch_Ellison_Data_All_UPitt_2015_to_2020/ChIP_seq_Analysis_Docker_started_on_11012020/sc_bam/H2Bub_subset_bams/ME052_S4_Spt6_AID_IAA_H2BK123ub_3_subsample_sorted.bam",
#                          annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
...
write.table(x=data.frame(fCount1$annotation[,c("GeneID")],fCount1$counts,fCount2$counts,fCount4$counts,fCount5$counts,
                         stringsAsFactors=FALSE),file="counts.txt",
                         quote=FALSE,sep="\t",row.names=FALSE)


conditions <- c("H2Bub_DMSO", "H2Bub_DMSO",
                "H2Bub_IAA", "H2Bub_IAA")
...
rownames(coldata) <- (c("H2Bub_DMSO", "H2Bub_DMSO",
                        "H2Bub_IAA", "H2Bub_IAA"))
...
#dds <- DESeq(dds)
resultsNames(dds)
sizeFactors(dds) <- c(1/4.30736, 1/4.39508, 1/3.32821, 1/3.79419)

dds = estimateDispersions(dds) 
dds = nbinomWaldTest(dds)

#!!!input contrasting conditions here 
contrast=c("condition","H2Bub_IAA","H2Bub_DMSO")
H2Bub_IAA_vs_H2Bub_DMSO <- results(dds, contrast, pAdjustMethod = "fdr")

write.table(subset(H2Bub_IAA_vs_H2Bub_DMSO, padj < 0.05), file="H2Bub_IAA_vs_H2Bub_DMSO_significant.tsv", 
            sep = "\t", quote = FALSE, row.names = TRUE, col.names = TRUE)


write.table(subset(H2Bub_IAA_vs_H2Bub_DMSO, log2FoldChange < -0.58), file="H2Bub_IAA_vs_H2Bub_DMSO_l2fc_less_than_-0.58.tsv", 
            sep = "\t", quote = FALSE, row.names = TRUE, col.names = TRUE)



#output is a results file comparing two conditions
png("H2Bub_IAA_vs_H2Bub_DMSO_MA_plot.png", height = 600, width = 600)
plotMA(H2Bub_IAA_vs_H2Bub_DMSO, ylim=c(-5,5), alpha = 0.05, xlab = "NCIS Normalized Read Counts")
dev.off()
...
pairs.panels(log2(ddsdf), xlim = c(4,16), ylim = c(4,16),
