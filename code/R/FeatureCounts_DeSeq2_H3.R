...
fCount1 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Mitch_Ellison_Data_All_UPitt_2015_to_2020/ChIP_seq_Analysis_Docker_started_on_11012020/sc_bam/H3_subset_bams/ME122_S8_Spt6_AID_DMSO_H3_1_subsample_sorted.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
fCount2 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Mitch_Ellison_Data_All_UPitt_2015_to_2020/ChIP_seq_Analysis_Docker_started_on_11012020/sc_bam/H3_subset_bams/ME116_S2_Spt6_AID_DMSO_H3_2_subsample_sorted.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
fCount3 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Mitch_Ellison_Data_All_UPitt_2015_to_2020/ChIP_seq_Analysis_Docker_started_on_11012020/sc_bam/H3_subset_bams/ME123_S9_Spt6_AID_IAA_H3_1_subsample_sorted.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
fCount4 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Mitch_Ellison_Data_All_UPitt_2015_to_2020/ChIP_seq_Analysis_Docker_started_on_11012020/sc_bam/H3_subset_bams/ME066_S18_Spt6_AID_IAA_H3_2_subsample_sorted.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
...
rownames(coldata) <- (c("H3_DMSO", "H3_DMSO",
                        "H3_IAA", "H3_IAA"))
...
#dds <- DESeq(dds)
resultsNames(dds)
sizeFactors(dds) <- c(1/5.97232, 1/6.74805, 1/5.58332, 1/4.41207)

dds = estimateDispersions(dds) 
dds = nbinomWaldTest(dds)

#!!!input contrasting conditions here 
contrast=c("condition","H3_IAA","H3_DMSO")
H3_IAA_vs_H3_DMSO <- results(dds, contrast, pAdjustMethod = "fdr")

write.table(subset(H3_IAA_vs_H3_DMSO, padj < 0.05), file="H3_IAA_vs_H3_DMSO_significant.tsv", 
            sep = "\t", quote = FALSE, row.names = TRUE, col.names = TRUE)


write.table(subset(H3_IAA_vs_H3_DMSO, log2FoldChange < -0.58), file="H3_IAA_vs_H3_DMSO_l2fc_less_than_-0.58.tsv", 
            sep = "\t", quote = FALSE, row.names = TRUE, col.names = TRUE)



#output is a results file comparing two conditions
png("H3_IAA_vs_H3_DMSO_MA_plot.png", height = 600, width = 600)
plotMA(H3_IAA_vs_H3_DMSO, ylim=c(-5,5), alpha = 0.05, xlab = "NCIS Normalized Read Counts")
dev.off()
