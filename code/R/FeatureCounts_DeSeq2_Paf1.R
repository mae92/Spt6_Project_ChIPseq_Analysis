...
fCount1 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Mitch_Ellison_Data_All_UPitt_2015_to_2020/ChIP_seq_Analysis_Docker_started_on_11012020/sc_bam/Spt6_AID_EF_subset_bams/ME017_S1_Spt6_AID_DMSO_HSV_Paf1_1_subsample_sorted.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
fCount2 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Mitch_Ellison_Data_All_UPitt_2015_to_2020/ChIP_seq_Analysis_Docker_started_on_11012020/sc_bam/Spt6_AID_EF_subset_bams/ME019_S3_Spt6_AID_DMSO_HSV_Paf1_2_subsample_sorted.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
fCount3 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Mitch_Ellison_Data_All_UPitt_2015_to_2020/ChIP_seq_Analysis_Docker_started_on_11012020/sc_bam/Spt6_AID_EF_subset_bams/ME018_S2_Spt6_AID_IAA_HSV_Paf1_1_subsample_sorted.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
fCount4 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Mitch_Ellison_Data_All_UPitt_2015_to_2020/ChIP_seq_Analysis_Docker_started_on_11012020/sc_bam/Spt6_AID_EF_subset_bams/ME020_S4_Spt6_AID_IAA_HSV_Paf1_2_subsample_sorted.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
...
conditions <- c("Paf1_DMSO", "Paf1_DMSO",
                "Paf1_IAA", "Paf1_IAA")
...
rownames(coldata) <- (c("Paf1_DMSO", "Paf1_DMSO",
                        "Paf1_IAA", "Paf1_IAA"))
...
#dds <- DESeq(dds)
resultsNames(dds)
sizeFactors(dds) <- c(1/6.37576, 1/3.88383, 1/1.65086, 1/1.70206)

dds = estimateDispersions(dds) 
dds = nbinomWaldTest(dds)

#!!!input contrasting conditions here 
contrast=c("condition","Paf1_IAA","Paf1_DMSO")
Paf1_IAA_vs_Paf1_DMSO <- results(dds, contrast, pAdjustMethod = "fdr")

write.table(subset(Paf1_IAA_vs_Paf1_DMSO, padj < 0.05), file="Paf1_IAA_vs_Paf1_DMSO_significant.tsv", 
            sep = "\t", quote = FALSE, row.names = TRUE, col.names = TRUE)


write.table(subset(Paf1_IAA_vs_Paf1_DMSO, log2FoldChange < -0.58), file="Paf1_IAA_vs_Paf1_DMSO_l2fc_less_than_-0.58.tsv", 
            sep = "\t", quote = FALSE, row.names = TRUE, col.names = TRUE)



#output is a results file comparing two conditions
png("Paf1_IAA_vs_Paf1_DMSO_MA_plot.png", height = 600, width = 600)
plotMA(Paf1_IAA_vs_Paf1_DMSO, ylim=c(-5,5), alpha = 0.05, xlab = "NCIS Normalized Read Counts")
dev.off()
